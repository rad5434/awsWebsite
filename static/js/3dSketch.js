        ...
        if(count!=num){ //count is the nth boat that is currently being drawn
            if(drawingSketches){
                ...
                if(len<sides[0].length){    //need to do 3 lines per iteration (trianlge). currently we only have 1
                    ...
                    p.line(vertices[0][sides[0][len]],vertices[1][sides[0][len]]*10+200,vertices[2][sides[0][len]],vertices[0][sides[1][len]],vertices[1][sides[1][len]]*10+200,vertices[2][sides[1][len]])
                    p.line(vertices[0][sides[1][len]],vertices[1][sides[1][len]]*10+200,vertices[2][sides[1][len]],vertices[0][sides[2][len]],vertices[1][sides[2][len]]*10+200,vertices[2][sides[2][len]])
                    p.line(vertices[0][sides[2][len]],vertices[1][sides[2][len]]*10+200,vertices[2][sides[2][len]],vertices[0][sides[0][len]],vertices[1][sides[0][len]]*10+200,vertices[2][sides[0][len]])
                    len+=1;
                    console.log(vertices[0][sides[0][len]],vertices[1][sides[0][len]]*10+200,vertices[2][sides[0][len]],vertices[0][sides[1][len]],vertices[1][sides[1][len]]*10+200,vertices[2][sides[1][len]])
                    console.log(vertices[0][sides[1][len]],vertices[1][sides[1][len]]*10+200,vertices[2][sides[1][len]],vertices[0][sides[2][len]],vertices[1][sides[2][len]]*10+200,vertices[2][sides[2][len]])
                    console.log(vertices[0][sides[2][len]],vertices[1][sides[2][len]]*10+200,vertices[2][sides[2][len]],vertices[0][sides[0][len]],vertices[1][sides[0][len]]*10+200,vertices[2][sides[0][len]])
