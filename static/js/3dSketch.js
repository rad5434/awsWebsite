        ...
        if(count!=num){ //count is the nth boat that is currently being drawn
            if(drawingSketches){
                ...
                if(len<sides[0].length){    //need to do 3 lines per iteration (trianlge). currently we only have 1
                    ...
                    p.line(vertices[0][sides[0][len]]*10+5000,vertices[1][sides[0][len]]*10+5000,vertices[2][sides[0][len]]*10+5000,vertices[0][sides[1][len]]*10+5000,vertices[1][sides[1][len]]*10+5000,vertices[2][sides[1][len]]*10+5000)
                    p.line(vertices[0][sides[1][len]]*10+5000,vertices[1][sides[1][len]]*10+5000,vertices[2][sides[1][len]]*10+5000,vertices[0][sides[2][len]]*10+5000,vertices[1][sides[2][len]]*10+5000,vertices[2][sides[2][len]]*10+5000)
                    p.line(vertices[0][sides[2][len]]*10+5000,vertices[1][sides[2][len]]*10+5000,vertices[2][sides[2][len]]*10+5000,vertices[0][sides[0][len]]*10+5000,vertices[1][sides[0][len]]*10+5000,vertices[2][sides[0][len]]*10+5000)
                    len+=1;
                }
                else{
                  len=0;
